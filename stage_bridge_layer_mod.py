...
class StageBridgeLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        if self._phase == 'TRAIN':
            ...
            top[8].reshape(1, 21 * 21)
            self._top_name_map['init_states'] = 8
    ...
    def forward_train(self, bottom, top):
        ...
        init_state0 = np.vstack((init_state0, init_state0[0:gt_boxes.shape[0]]))

        labels, rois_out, fg_inds, keep_inds, mask_targets, top_mask_info, bbox_targets, bbox_inside_weights, init_states = \
            self._sample_output(all_rois, gt_boxes, im_info[2], gt_masks, mask_info, init_state0)
    ...
    def _sample_output(self, all_rois, gt_boxes, im_scale, gt_masks, mask_info, init_state0):
