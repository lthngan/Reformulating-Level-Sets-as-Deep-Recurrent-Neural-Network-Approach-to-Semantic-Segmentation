...
class ProposalLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        if not cfg.TEST.HAS_RPN:
            rois_info = bottom[4].data[0, :, :]
            no_rois = rois_info.shape[0]
            proposals[0:no_rois, 1:5] = rois_info[0:no_rois, 1:5]
            scores[0:no_rois] = 1
            scores[no_rois:-1] = 0
        ...
        if str(self.phase) == 'TEST':
            init_state0 = bottom[3].data
            init_states = np.repeat(init_state0, proposals.shape[0], axis=0)
            blobs['init_states'] = init_states
    ...
    def backward(self, top, propagate_down, bottom):
        ...
        if propagate_down[1]:
            ...
            dfdxc = top[0].diff[top_non_zero_ind, 1]
            dfdyc = top[0].diff[top_non_zero_ind, 2]
            dfdw = top[0].diff[top_non_zero_ind, 3]
            dfdh = top[0].diff[top_non_zero_ind, 4]
